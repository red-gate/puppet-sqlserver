...
define sqlserver::v2014::instance(
  ...
  $registry_instance_path = "SOFTWARE\\Microsoft\\Microsoft SQL Server\\MSSQL12.${instance_name}"
  ...
  exec { "Install SQL Server instance: ${instance_name}":
    command => "\"${::sqlserver::v2014::iso::installer}\" \
/QUIET \
...
/FILESTREAMSHARENAME=${instance_name}",
  ...
  if $install_type == 'Patch' {
    ...
    exec { "Install SQL Server Patch instance: ${instance_name}":
      command => "\"${::sqlserver::v2014::patch::installer}\" \
/QUIET \
/IACCEPTSQLSERVERLICENSETERMS \
/IACCEPTROPENLICENSETERMS \
/ACTION=Patch \
/INSTANCENAME=${instance_name}",
      unless  => "cmd.exe /C reg query ${get_patchlevel_from_registry} | findstr ${::sqlserver::v2014::patch::version}",
      require => [
        Exec["Install SQL Server instance: ${instance_name}"],
