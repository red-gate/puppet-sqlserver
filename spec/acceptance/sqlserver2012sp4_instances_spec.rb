...
['SQL2012_1', 'SQL2012_2'].each do |instance_name|
  ...
  describe windows_registry_key("HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Microsoft SQL Server\\MSSQL11.#{instance_name}\\Setup") do
    it { should exist }
    it { should have_property_value('PatchLevel', :type_string, '11.4.7001.0') }
  end
end

describe windows_registry_key("HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Microsoft SQL Server\\MSSQL11.SQL2012_1\\Mssqlserver\\Supersocketnetlib") do
...
describe command('sqlcmd -s localhost\SQL2012_1 -Q "select name from sys.databases"') do
