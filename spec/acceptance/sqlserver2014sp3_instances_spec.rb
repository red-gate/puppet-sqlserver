...
['SQL2014_1', 'SQL2014_2'].each do |instance_name|
  ...
  describe windows_registry_key("HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Microsoft SQL Server\\MSSQL12.#{instance_name}\\Setup") do
    it { should exist }
    it { should have_property_value('PatchLevel', :type_string, '12.3.6024.0') }
  end
end

describe windows_registry_key("HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Microsoft SQL Server\\MSSQL12.SQL2014_1\\Mssqlserver\\Supersocketnetlib") do
...
describe command('sqlcmd -s localhost\SQL2014_1 -Q "select name from sys.databases"') do
